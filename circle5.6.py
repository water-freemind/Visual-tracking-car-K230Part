...
WHITE_TRACK_THRESHOLD = [(78, 100, -15, 15, -15, 15)]#
RED_TRACK_THRESHOLD = [(0, 63, 34, 127, -51, 127)]#
...
CARD_THRESHOLD = [(0, 180, -30, 30, 0, 50)]     # 黑色卡片阈值
...
# 起点矩形检测参数
RECTANGLE_ROI = (46, 61, 211, 65)  # 起点矩形出现的区域（x,y,w,h），需要根据实际调整
RECTANGLE_MIN_AREA = 5000  # 矩形最小面积（像素），大于普通轨道（需要调）
RECTANGLE_MAX_AREA = 10000  # 矩形最大面积（像素）（需要调）
RECTANGLE_ASPECT_RATIO = (2.7, 5.9)
first_lap = True  # 首次运行标志（首次无需检测矩形）
rect_detected = 0  # 矩形检测状态
lap_counted = False  # 圈数计数控制。标记当前圈是否已计数（防止同一圈多次检测到矩形而重复计数）
...
def detect_start_rectangle(binary_img):
    ...
    blobs = rect_roi_img.find_blobs(WHITE_TRACK_THRESHOLD, merge=True)
...
def line_following_control(binary_img, img_width, img):
    ...
    # 新增权重分配
    scan_weights = [0.8, 1.3, 1.5, 1.3, 0.8] #权重分配参数

    img_center_x = img_width // 2
    valid_centers = []  # 收集有效扫描线的轨道中心
    valid_weights = []  # 新增有效扫描线的对应权重
    ...
    for idx, scan_y in enumerate(scan_ys):
            ...
            if left is not None and right is not None:
                # 计算当前扫描线的轨道中心
                current_center = (left + right) // 2
                valid_centers.append(current_center)
                valid_weights.append(scan_weights[idx])  # 绑定对应权重
    ...
    # 处理有效中心
    if valid_centers and valid_weights:

        # 计算加权平均中心
        weighted_sum = sum(center * weight for center, weight in zip(valid_centers, valid_weights))
        total_weight = sum(valid_weights)
        avg_center = int(weighted_sum / total_weight) # 加权平均中心
...
def count_blob_vertices(blob):
    ...
    #print(aspect_ratio,density)
    # 基于长宽比的简单判断
    # 1. 优先判断正方形：宽高比非常接近 1.0
    # 可以根据实际卡片的规整程度微调这个范围
    if 1.2 < aspect_ratio < 1.5 and density > 0.5:
        return 4  # 正方形

    # 2. 然后判断线段：宽高比非常大或非常小
    # 宽度远大于高度，或者高度远大于宽度
    if aspect_ratio < 1.4 and 0.35< density < 0.5:
        return 2  # 线段
...
try:
    ...
    while True:
        ...
        line_str = b"dist=100\nstatus=1\nnum=5000\n"
        #串口信息
        User_Data = [current_lap,required_laps,current_circle,required_circles,int(dis_err),rect_detected,status]#现圈数,需求圈数，现环岛数,需求环岛数，偏差值,起始点,环岛标志
        # 图像处理（增加断轨修复）
