...
WHITE_TRACK_THRESHOLD = [(38, 100, -128, 2, -20, 78)]
RED_TRACK_THRESHOLD = [(0, 53, 10, 127, -26, 127)]
...
# --- 卡片检测相关参数 ---
CARD_DETECTION_ROI = (46, 61, 211, 65)  # 卡片检测区域
...
def count_blob_vertices(blob):
    ...
    # 可以根据实际卡片的规整程度微调这个范围
    if 1.2 < aspect_ratio < 1.5 and density<0.4:
        return 4  # 正方形

    # 2. 然后判断线段：宽高比非常大或非常小
    # 宽度远大于高度，或者高度远大于宽度
    if aspect_ratio < 1.3 and 0.5< density < 0.65:
        return 2  # 线段
...
try:
    ...
    while True:
        ...
        img.draw_string_advanced(10, 10, 15, "x:0-319 y:0-239", (255,0,0))  # 摄像头分辨率320x240
        # 2. 显示所有符合轨道阈值的色块信息（面积、宽高比）
        binary_img = img.binary(WHITE_TRACK_THRESHOLD)
        blobs = binary_img.find_blobs(WHITE_TRACK_THRESHOLD, merge=True)
        if blobs:
            for i, blob in enumerate(blobs[:3]):  # 显示前3个最大色块
                x, y, w, h = blob.rect()
                area = blob.pixels()  # 色块像素面积
                aspect_ratio = w/h  # 宽高比
                print(area,aspect_ratio)
                # 在图像上画色块框+显示信息
                img.draw_rectangle((x,y,w,h), color=(0,255,0))
                img.draw_string_advanced(x, y-10, 12, f"A:{area} R:{aspect_ratio:.2f}", (0,255,0))
