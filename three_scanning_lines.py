...
def find_edges_on_line(binary_img, scan_y, roi_x, roi_width, img_center_x):
    ...
    for x in range(roi_x, img_center_x + 30):
        if binary_img.get_pixel(x, scan_y)[0] > 0:
            left_edge_x = x
            break

    # 从ROI最右边向左扫描（放宽至中心左侧50像素）
    for x in range(roi_x + roi_width - 1, img_center_x - 30, -1):
        if binary_img.get_pixel(x, scan_y)[0] > 0:
            right_edge_x = x
            break
...
def line_following_control(binary_img, img_width, img):
    ...
    # 改为两条扫描线（可根据需要调整比例）
    scan_ys = [ROI[1] + int(ROI[3] * p) for p in [0.2,0.6, 1.0]]  # 仅保留两条扫描线
